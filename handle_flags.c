...
char	*handle_negative_num(char *str, t_arg mod)
{
	...
	else if (mod.width > ft_strlen(str) && g_nb < 0)
	{
		if (!mod.precision)
			str = ft_strjoin("-", str);
		size = mod.width - ft_strlen(str);
...
char	*handle_width_flag_minus(char *str, t_arg mod)
{
	...
	if (mod.width > ft_strlen(str) && !mod.precision && g_nb < 0)
		str = handle_negative_num(str, mod);
	else if (mod.width > ft_strlen(str) && !mod.precision && g_nb >= 0)
	{
	...
	else if (mod.width > ft_strlen(str) && mod.precision < mod.width)
	{
		tmp = ft_memalloc(mod.width - ft_strlen(str) + 1);
		tmp = ft_memset(tmp, ' ', mod.width - ft_strlen(str));
...
char	*handle_width_flag_zero(char *tmp, char *str, t_arg mod)
{
	...
	{
		tmp = ft_memalloc(mod.width - ft_strlen(str) + 1);
		tmp = ft_memset(tmp, '0', mod.width - ft_strlen(str) - 1);
		str = ft_strjoin(tmp, str);
		if (!mod.precision)
			str = ft_strjoin("-", str);
	}
	else if (mod.width > ft_strlen(str) && mod.width && mod.precision)
...
char	*handle_int_width(char *str, t_arg mod)
{
	...
	else if (mod.width <= ft_strlen(str) && g_nb < 0 && !mod.flag_minus)
	{
